...
## How To Have Naive Auth
- how to have super simple "auth" in order to build the rest of your app

When the login form is submitted, make a fetch to our API, update state

X PASSWORD are stores as plain text!!
- Log in doesn't persists :( require tokens
...
# a good time? component did mount
- When the app is loaded, make a fetch call
  find a way to send the token in the headers via a fetch
    - server is going to send user
      - currentUser state is updated appropriately 
